...
sub process_entities_html {
    ...
    $content =~ s{src="$wikih->{siteinfo}->{imagepath}/math(/.+?)"}{process_generated_media($wikih, "math", $1)}ges;

    # And any thumbnails
    $content =~ s{src="$wikih->{siteinfo}->{imagepath}/thumb(/.+?)"}{process_generated_media($wikih, "thumb", $1)}ges;
