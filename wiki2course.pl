...
## @fn $ fix_popup_wikitext($wikih, $b64body);
...
sub fix_popup_wikitext {
    ...
    return '<span class="twpopup-inner">'.encode_base64(fix_wikitext($wikih, decode_base64($b64body))).'</span>';
}


## @fn $ fix_wikitext($wikih, $content)
...
sub fix_wikitext {
    ...
    # Fix popups
    $content =~ s{<span class="twpopup-inner">(.*?)</span>}{fix_popup_wikitext($wikih, $1, $2)}ges;
...
sub process_entities_html {
    ...
    return fix_wikitext($wikih, $content);
}
