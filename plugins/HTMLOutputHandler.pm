...
## @method $ metadata_validate($themedir, $data, $plugins)
...
# @param plugins   The global plugins hash.
...
sub metadata_validate {
    ...
    die "FATAL: metadata_validate: $shortname/metadata.xml missing theme title." if(!$data -> {"title"});
    die "FATAL: metadata_validate: $shortname/metadata.xml missing theme name."  if(!$data -> {"name"});
    die "FATAL: metadata_validate: $shortname/metadata.xml missing theme index order."  if(!$data -> {"indexorder"});
    
    # Check modules
    foreach my $module (keys(%{$data -> {"module"}})) {
        ...
        die "FATAL: metadata_validate: $shortname/metadata.xml missing module title for '$module'." if(!$data -> {"module"} -> {$module} -> {"title"});
        die "FATAL: metadata_validate: $shortname/metadata.xml missing module level for '$module'." if(!$data -> {"module"} -> {$module} -> {"level"});
        die "FATAL: metadata_validate: $shortname/metadata.xml missing module index order for '$module'." if(!$data -> {"module"} -> {$module} -> {"indexorder"});

        # check that any prereqs are valid.
        if($data -> {"module"} -> {$module} -> {"prerequisites"}) {
            my $entries;
            eval { $entries = $data -> {"module"} -> {$module} -> {"prerequisites"} -> {"target"}; };
            ...
            foreach my $entry (@$entries) {
                die "FATAL: metadata_validate: $shortname/metadata.xml contains unknown prerequisite '$entry' for '$module'." 
                    if(!$data -> {"module"} -> {$entry});
            }
        }
        
        # check that any prereqs are valid.
        if($data -> {"module"} -> {$module} -> {"leadsto"}) {
            my $entries;
            eval { $entries = $data -> {"module"} -> {$module} -> {"leadsto"} -> {"target"}; };
...
## @method $ load_metadata($themedir, $validate, $plugins)
...
# are met.
#
# @param themedir  The theme directory to load metdata from.
# @param validate  If true, the metadata is validated to ensure it contains 
#                  required sections
# @param plugins   A reference to the global plugins hash.
...
sub load_metadata {
    ...
    if(-e "$themedir/metadata.xml") {
        $data = XMLin("$themedir/metadata.xml");

        # If we need to validate the metadata, go ahead and do so.
        if($validate) {
            print Data::Dumper -> Dump([$data], ['*data']) if($self -> {"verbose"} > 1);
            return 0 if(!$self -> metadata_validate($themedir, $data, $plugins));
        }
    } else {
        return 1;
    }
