...
package WebUI::System::WikiSupport;
...
# Create a new WikiSupport object
#
...
## @method private $ _metadata_find($wikiconfig, $page)
...
## @fn private void _metadata_filters($metadata, $filterhash)
...
## @method $ check_wiki_login($username, $password, $wikiconfig)
...
sub check_wiki_login {
    ...
    my $mw = MediaWiki::API -> new({ api_url => $wikiconfig -> {"wiki2course"} -> {"api_url"} })
        or return $self -> self_error("Unable to create new MediaWiki API object.");
...
## @method private $ _wiki_transclude($wikih, $page, $templatestr)
...
# @return The expanded template text on success, undef on error.
sub _wiki_transclude {
    my $self        = shift;
    my $wikih       = shift;
    my $pagename    = shift;
    ...
    my $response = $wikih -> api({ action => 'expandtemplates',
                                   ...
                                   prop   => 'revisions',
                                   text   => $templatestr} )
        or return $self -> self_error("Unable to process transclusion in page $pagename. Error from the API was:".$wikih->{"error"}->{"code"}.': '.$wikih->{"error"}->{"details"});
...
sub get_course_filters {
    ...
    $self -> _metadata_filters($self -> _metadata_find($wikiconfig, $coursedata), $filterhash);
    ...
    foreach my $theme (@themes) {
        ...
        $self -> _metadata_filters($self -> _metadata_find($wikiconfig, $page), $filterhash);
    }
...
sub get_wikiconfig_hash {
    ...
    opendir(CONFDIR, $self -> {"settings"} -> {"config"} -> {"wikiconfigs"})
        or return $self -> self_error("Unable to open wiki configuration dir: ".$self -> {"dbh"} -> errstr);
    ...
    while(my $entry = readdir(CONFDIR)) {
        ...
        my $config = $self -> get_wiki_config($entry)
            or return undef;
...
sub get_wiki_config {
    ...
    my $config = ConfigMicro -> new(path_join($self -> {"settings"} -> {"config"} -> {"wikiconfigs"}, $config_name))
        or $self -> self_error("Unable to open wiki configuration $config_name: ".$Webperl::SystemModule::errstr);
